...
However after the more thorough analysis of performance of Hyperbola Quintessence finally I came to a conclusion that is suffers from another bottleneck: __register spilling__. This is because this small calculation uses quite many variables and compiler is not able to put them all in CPU registers, so it is forced to put them on stack.
<!-- -->
