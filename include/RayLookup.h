...
public:
	...
	constexpr ALWAYS_INLINE Bitboard GetRayInDir(const int pos, const DirLookup::Direction dir) const __restrict__
	{
        ...
        const auto* __restrict__ idxLookup = (const uint16_t* __restrict__ ) &(this->idxLookup);
		const auto idx = idxLookup[((uint32_t)dir) * 64 + pos];
		const Bitboard* __restrict__ maskLookup = (const Bitboard* __restrict__ ) &(this->maskLookup);
		#else		
